...
export const useLadder = () => {
     ...
     */
    const startDrawing = useCallback((startX: number, color: string, onFinish: (finalX: number) => void, speed: number = 50) => {
            ...
            switch (nodeInfo?.draw) {
                case "LEFT_TO_RIGHT":
                    ...
                    setTimeout(() => {
                        ...
                        strokeLine(ctx, nextX, y, "h", "d", color, 3);
     ...
     */
    const drawPathSync = useCallback((startX: number, color: string) => {
        const ctx = canvasRef.current?.getContext("2d");
        if (!ctx) return;
